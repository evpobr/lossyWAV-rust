...
double Fill_FFT_With_Warped_Spectrum_Cubic()
{
    ...
    for (int32_t ts_i = 0; ts_i <= SGNS.FFT.length_half; ts_i++)
    {
        ...
        FFT_Array.DComplex[ts_i + ((SGNS.FFT.length - 2 * ts_i) * ((ts_i & SGNS.FFT.length_half_m1) > 0))] = tDComplex(ts_y, 0.);

        FFT_Array.DComplex[ts_i] = tDComplex(ts_y, 0.);
...
double Fill_FFT_With_Warped_Spectrum_Default()
{
    ...
    for (int32_t ts_i = 0; ts_i <= SGNS.FFT.length_half; ts_i++)
    {
        ...
        FFT_Array.DComplex[ts_i + ((SGNS.FFT.length - ts_i - ts_i) * ((ts_i & SGNS.FFT.length_half_m1) > 0))] = tDComplex(ts_y, 0.);
...
double Make_Filter(int32_t this_channel)
{
    ...
    if ((parameters.shaping.active) && (!parameters.shaping.fixed))
    {
        ...
        if (SGNS.Control.Fill_FFT_With_Warped_Spectrum())
        {
            ...
            {
                SGNS.Filters[this_channel].LD.r[ts_i] = FFT_Array.DComplex[ts_i].real();
            }
...
{
    tDComplex y = tDComplex_complex_exp(Freq);
...
void nSGNS_Initialise()
{
    ...
    for (int32_t ts_i = 0; ts_i <= SGNS.Filter_Order; ts_i++)
    {
        SGNS.Filters[MAX_CHANNELS].LD.r[ts_i] = FFT_Array.DComplex[ts_i].real();
    }
